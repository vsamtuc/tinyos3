...
#define min(x,y) (x<y)?x:y
	memcpy(info->args, pcb->args, min(pcb->argl,PROCINFO_MAX_ARGS_SIZE));
#undef min
}

int procinfo_read(void* sobj, char* buf, unsigned int size)
{
	procinfo_stream* info = sobj;

	unsigned int get_data()
	{		
		if(info->pos == sizeof(procinfo)) {
			/* Load data */
			PCB* pcb=NULL;
			/* Get next process, if any */
			while(pcb==NULL && info->curpid < MAX_PROC) { 
				info->curpid++;
				pcb = get_pcb(info->curpid);

				if(pcb) {
					/* Ok, found one, load it */
					load_procinfo(&info->info, pcb);
					info->pos = 0;
				}
			} 
		}
		return sizeof(procinfo)-info->pos;
	}

	/* count the bytes returned */
	int count=0;

	while(1) {
		/* bytes available */
		unsigned int dsize = get_data();

		/* bytes to transfer */
		unsigned int tsize = (dsize<size) ? dsize : size;

		if(tsize == 0) break;
		memcpy(buf, info->buffer+info->pos, tsize);
		buf += tsize;
		size -= tsize;
		info->pos = tsize;
		count += tsize;
...
Fid_t sys_OpenInfo()
{
	...
	procinfo_stream* obj = xmalloc(sizeof(procinfo_stream));
	obj->curpid = 0;
	obj->pos = sizeof(procinfo);
